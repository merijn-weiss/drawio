(function()
{
	...
	Sidebar.prototype.addMondrianPalette = function()
	{
		...
		for(let sidebarKey in mondrianConfig.Sidebars)
		{
			...
			// Extend More Shapes...
			sideBars[sideBar.id].entries.push({title: sideBar.lib, id: sideBar.id + sideBar.lib, image: 'mondrianExtension/' + IMAGE_PATH + '/sidebar-mondrian-base-shapes.png'});
		}
	...
	Sidebar.prototype.GenerateMondrianPalette = function(sideBarConfigs)
	{
		...
		for(let sidebarConfigKey in sideBarConfigs)
		{
			...
			{
				let isIconAll = (sideBarConfig.lib === "Icons (All)"); // DIRTY HACK TO FILL ICON LIST
				...
				for (let i = 0; i < stencilKeysLength; i++)
				{
					...
					if(stencilKey.startsWith(namespace))
					{
						...
						if(isIconAll)// DIRTY HACK TO FILL ICON LIST
							window.MONDRIAN_REPO.addIcon(newStencilSection, stencilName.toLowerCase());
